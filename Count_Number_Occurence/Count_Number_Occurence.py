...
def search3(list,pattern):
    fullcount=list.count(pattern) #buildtin phtyon fucntion to count number of orruences
    return fullcount
text=readfile(f)
...
answer=search3(sortedlist,my_pattern.lower()) #input "a,A" --> answer should be 7 || input is lowercase just as the list
if answer>0:
